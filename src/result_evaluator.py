...
"""
        ...
        for key, title in viz_titles.items():
            ...
            <div class=\"visualization\">
                <h3>{title}</h3>
                <img src=\"{files[key].name}\" alt=\"{title}\" style=\"max-width: 400px; height: auto; display: block; margin: 0 auto;\">
            </div>
        """
...
"""
        ...
        if log_path.exists():
            ...
            # Remove problematic emoji encodings
            log_content = log_content.encode('utf-8', errors='replace').decode('utf-8', errors='replace')
            log_content = log_content.replace('ðŸ”¬', '').replace('ðŸ“Š', '').replace('ðŸ“', '').replace('ðŸ“', '')
            html += f'''
    <div class="visualization">
        <h3>EVALLab Agent Execution Log for this Research Paper</h3>
        <details style="white-space:pre-wrap; background:#f8f8f8; border:1px solid #ccc; padding:10px; border-radius:6px; max-height:400px; overflow:auto;">
            <summary>Show/Hide Agent Log</summary>
            <pre style="font-size:12px;">{html_module.escape(log_content)}</pre>
        </details>
    </div>
    <div class="footer">
        <p>Generated by EVALLab</p>
